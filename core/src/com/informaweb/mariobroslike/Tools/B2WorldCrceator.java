...
public class B2WorldCrceator {
    public B2WorldCrceator(PlayScreen screen) {
        World world = screen.getWorld(); // On récupère le monde
        TiledMap map = screen.getMap(); // On récupère la carte
        ...
        for(MapObject object : map.getLayers().get(3).getObjects().getByType(RectangleMapObject.class)) {
            ...
            shape.setAsBox(rect.getWidth() / 2 /MarioBrosLike.PPM, rect.getHeight() / 2 / MarioBrosLike.PPM); // On indique la taille de l'objet
            fdef.shape = shape; // On indique la forme de l'objet
            fdef.filter.categoryBits = MarioBrosLike.OBJECT_BIT; // On indique la catégorie de l'objet
            body.createFixture(fdef); // On crée l'objet dans le monde
        }
        ...
        for(MapObject object : map.getLayers().get(5).getObjects().getByType(RectangleMapObject.class)) {
            ...
            new Brick(screen, rect); // On crée l'objet dans le monde
        ...
        for(MapObject object : map.getLayers().get(4).getObjects().getByType(RectangleMapObject.class)) {
            ...
            new Coin(screen, rect); // On crée l'objet dans le monde
        }
    }
